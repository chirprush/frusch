...
void test_context_start(test_context *context, size_t length) {
	size_t test_length = length ? length : context->length;
	printf("%sRunning\e[0m %s%ld\e[0m %stests\e[0m:", PURPLE, YELLOW, test_length, PURPLE);
}
...
void test_context_end(test_context *context) {
	printf("%sResults\e[0m: %s%d passed\e[0m, %s%d failed\e[0m\n", PURPLE, GREEN, context->passed, RED, context->failed);
}
